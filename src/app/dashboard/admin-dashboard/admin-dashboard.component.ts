...
export class AdminDashboardComponent implements OnInit {
  ...
  public meetingReminder(): any {
    ...
    for (let meetingEvent of this.meetings) {
      if (
        isSameDay(new Date(), meetingEvent.start) &&
        ...
        currentTime > new Date(meetingEvent.start).getTime() &&
        new Date(currentTime - meetingEvent.start).getTime() < 10000
      ) {
        this.toastr.info(
          `Meeting ${meetingEvent.meetingTopic} Started!`,
          `Gentle Reminder`
        );
      }
    }
  ...
  public getAllUsers = () => {
    //this function will get all the normal users from database.
  ...
  public sentMeetingRemindersonEmailFunction = () => {
    ...
    if (this.authToken != null && this.adminId != null) {
      this.appService
        .sentMeetingReminders(this.adminId, this.authToken)
        .subscribe(
          (apiResponse) => {
            if (apiResponse.status == 200) {
  ...
  public getUserAllMeetingFunction = () => {
    //this function will get all the meetings of User.

    if (this.receiverId != null && this.authToken != null) {
      this.appService
        .getUserAllMeeting(this.receiverId, this.authToken)
        .subscribe(
          (apiResponse) => {
            if (apiResponse.status == 200) {
          ...
          (error) => {
            if (error.status == 400) {
              this.toastr.warning(
                'Calendar Failed to Update',
                'Either user or Meeting not found'
              );
              this.events = [];
            } else {
  ...
  public deleteMeetingFunction(meeting): any {
    this.appService.deleteMeeting(meeting.meetingId, this.authToken).subscribe(
      (apiResponse) => {
        if (apiResponse.status == 200) {
  ...
  public authErrorFunction: any = () => {
    this.socketService.listenAuthError().subscribe((data) => {
      this.toastr.info('Missing Authorization Key', 'Please login again');
      this.router.navigate(['/user/login']);
    }); //end subscribe
