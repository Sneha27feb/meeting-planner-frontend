...
export class UpdateMeetingComponent implements OnInit {
  ...
  public validateDate(startDate: any, endDate: any): boolean {
    //method to validate the the start and end date of meeting .
  ...
  public validateCurrentDate(startDate: any): boolean {
    //method to validate the current date and date of meeting .
  ...
  public getMeetingDetailsFunction = (meetingId, authToken) => {
    //this function will get meeting details.

    this.appService.getMeetingDetails(meetingId, authToken).subscribe(
      //using the api get metting details.
      (apiResponse) => {
  ...
  public updateMeetingFunction(): any {
    if (!this.subject) {
      ...
      this.validateCurrentDate(this.startDate1) &&
      this.validateCurrentDate(this.endDate1)
    ) {
      this.toastr.warning(
        "Meeting can't be schedule in back date/time",
        'Warning!'
      );
    } else {
      let data = {
      ...
      this.appService.updateMeeting(data).subscribe(
        (apiResponse) => {
          if (apiResponse.status == 200) {
            this.toastr.success(
              'We emailed the final schedule to participant',
              'Meeting Rescheduled'
            );
